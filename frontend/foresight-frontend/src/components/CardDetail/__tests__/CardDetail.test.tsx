...
const mockCardData: Card = {
  ...
  description: 'This is a detailed description of the test card used for integration testing.',
...
vi.mock('../../StageBadge', () => ({
  ...
  StageProgress: ({ stage }: { stage: number }) => <div data-testid="stage-progress">Stage {stage}</div>,
}));
...
vi.mock('../../AnchorBadge', () => ({
  AnchorBadge: ({ anchor }: { anchor: string }) => <span data-testid="anchor-badge">{anchor}</span>,
}));
...
vi.mock('../../Top25Badge', () => ({
  ...
  Top25ExpandedBadge: ({ priorities }: { priorities: string[] }) => (
    <span data-testid="top25-expanded">{priorities?.length || 0} priorities</span>
  ),
}));
  ...
  const mockFrom = vi.fn((table: string) => {
    ...
    chainMock.select = vi.fn(() => {
      chainMock.eq = vi.fn((field: string, _value: string) => {
        ...
        if (table === 'sources') {
          chainMock.order = vi.fn(() => Promise.resolve({ data: mockSources, error: null }));
          return chainMock;
        }
        if (table === 'card_timeline') {
          chainMock.order = vi.fn(() => Promise.resolve({ data: mockTimeline, error: null }));
          return chainMock;
        }
        if (table === 'card_notes') {
          chainMock.or = vi.fn(() => ({
        ...
        if (table === 'research_tasks') {
          chainMock.eq = vi.fn(() => ({
            order: vi.fn(() => ({
              limit: vi.fn(() => Promise.resolve({ data: mockResearchTasks, error: null })),
            })),
          }));
          return chainMock;
        }
    ...
    chainMock.insert = vi.fn(() => ({
      select: vi.fn(() => ({
        single: vi.fn(() => Promise.resolve({
          data: { id: 'new-note', content: 'Test note', is_private: false, created_at: new Date().toISOString() },
          error: null
        })),
      })),
    }));
...
vi.mock('../../../lib/discovery-api', () => ({
  ...
  getRelatedCards: vi.fn(() => Promise.resolve({ related_cards: mockRelatedCards })),
}));
...
vi.mock('../../visualizations/TrendVelocitySparkline', () => ({
  ...
  TrendVelocitySparklineSkeleton: () => <div data-testid="velocity-sparkline-skeleton" />,
}));
...
describe('CardDetail', () => {
  ...
  describe('Overview Tab Content', () => {
    ...
    it('displays maturity score', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(String(mockCardData.maturity_score))).toBeInTheDocument();
      }, { timeout: 3000 });
    });
  ...
  describe('Action Buttons', () => {
    ...
    it('displays Deep Research button', async () => {
      ...
      await waitFor(() => {
        ...
        const buttons = screen.getAllByRole('button', { name: /Deep Research/i });
  ...
  describe('Custom ClassName', () => {
    it('applies custom className to container', async () => {
      render(
        <MemoryRouter initialEntries={['/cards/test-card-slug']}>
          <Routes>
            <Route path="/cards/:slug" element={<CardDetail className="custom-test-class" />} />
          </Routes>
        </MemoryRouter>
      );
