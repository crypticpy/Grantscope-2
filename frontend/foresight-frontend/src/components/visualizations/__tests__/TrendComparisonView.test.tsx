  ...
  return render(
    <MemoryRouter initialEntries={[`${route}?card_ids=${cardIds}`]}>
      <Routes>
        ...
        <Route path="/signals/:slug" element={<div>Card Detail Page</div>} />
      </Routes>
    </MemoryRouter>,
  );
}
...
describe("TrendComparisonView", () => {
  ...
  describe("Invalid Params State", () => {
    it("shows invalid state when no card_ids param", () => {
      ...
      expect(
        screen.getByText("Select Two Signals to Compare"),
      ).toBeInTheDocument();
    });
  ...
  describe("Successful Rendering", () => {
    ...
    it("renders stage progression comparison heading", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Stage Progression Comparison"),
        ).toBeInTheDocument();
      });
    });
  });
