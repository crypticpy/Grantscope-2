...
_TIER_SCORES = {1: 85, 2: 60, 3: 35}


def _compute_composite_score(domain_rep: DomainReputation) -> float:
    """Calculate composite score from tier and texas relevance bonus."""
    tier_score = _TIER_SCORES.get(domain_rep.curated_tier, 20)
    return tier_score * 0.50 + (domain_rep.texas_relevance_bonus or 0)
...
async def create_domain_reputation(
    ...
    try:
        ...
        domain_rep.composite_score = _compute_composite_score(domain_rep)
        db.add(domain_rep)
        await db.flush()
...
async def update_domain_reputation(
    ...
    try:
        ...
        # Recalculate composite score if contributing fields changed
        if "curated_tier" in data or "texas_relevance_bonus" in data:
            domain_rep.composite_score = _compute_composite_score(domain_rep)
