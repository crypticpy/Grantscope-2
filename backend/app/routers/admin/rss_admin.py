    ...
    def validate_url_scheme_and_host(cls, v: HttpUrl) -> HttpUrl:
        ...
        try:
            ...
            if "not allowed" in str(exc):
                raise
            # Not a literal IP — resolve hostname and check resolved addresses
            pass
            ...
            for family, _type, _proto, _canonname, sockaddr in addrinfos:
                ...
                for network in _BLOCKED_NETWORKS:
                    if resolved_ip in network:
                        raise ValueError(
                            f"URL hostname resolves to a private/loopback IP ({sockaddr[0]})"
                        )
        except socket.gaierror:
            # DNS resolution failed — could be a typo or an internal hostname
            # Allow the URL through; the actual fetch will fail later with a clear error
            pass
...
class RssFeedUpdate(BaseModel):
    ...
    @field_validator("url")
    @classmethod
    def validate_url_scheme_and_host(cls, v: Optional[HttpUrl]) -> Optional[HttpUrl]:
        if v is None:
            return v
        return RssFeedCreate.validate_url_scheme_and_host(v)
...
async def list_feeds(
    ...
    try:
        ...
        # Pre-compute recent item counts with a single grouped query
        seven_days_ago = datetime.now(timezone.utc) - timedelta(days=7)
        recent_counts_result = await db.execute(
            select(
                RssFeedItem.feed_id,
                func.count(RssFeedItem.id).label("recent_count"),
            )
            .where(RssFeedItem.created_at >= seven_days_ago)
            .group_by(RssFeedItem.feed_id)
        )
        recent_item_counts: dict = {
            str(row.feed_id): row.recent_count for row in recent_counts_result.all()
        }

        enriched = []
        for feed in feeds:
            feed_dict = _feed_to_dict(feed)
            feed_dict["recent_items_7d"] = recent_item_counts.get(str(feed.id), 0)
