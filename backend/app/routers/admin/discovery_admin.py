...
async def trigger_discovery_run(
    ...
    try:
        ...
        # Load persisted discovery config, starting from defaults
        persisted_result = await db.execute(
            select(SystemSetting).where(
                SystemSetting.key.like(f"{DISCOVERY_CONFIG_PREFIX}%")
            )
        )
        persisted_config: dict[str, Any] = dict(DISCOVERY_DEFAULTS)
        for row in persisted_result.scalars().all():
            persisted_config[row.key] = row.value

        # Build config: start with persisted, overlay request-body overrides
        config_data: dict[str, Any] = {
            "max_queries_per_run": persisted_config.get(
                "discovery.max_queries_per_run", 100
            ),
            "max_sources_total": persisted_config.get("discovery.total_cap", 500),
            "auto_approve_threshold": persisted_config.get(
                "discovery.auto_approve_threshold", 0.95
            ),
            "dry_run": persisted_config.get("discovery.dry_run", False),
        }
        # Apply explicit request-body overrides
        ...
        return {
            ...
            "triggered_by": "manual",
            "config": config_data,
            "message": "Discovery run started successfully. The worker will execute it shortly.",
        }
    except HTTPException:
        raise
    except Exception as e:
