...
class DateRange(BaseModel):
    ...
    start: Optional[date] = Field(
        None, description="Start date (inclusive) in YYYY-MM-DD format"
    )
    end: Optional[date] = Field(
        None, description="End date (inclusive) in YYYY-MM-DD format"
    )
...
class ScoreThreshold(BaseModel):
    ...
    min: Optional[int] = Field(
        None, ge=0, le=100, description="Minimum score value (0-100)"
    )
    max: Optional[int] = Field(
        None, ge=0, le=100, description="Maximum score value (0-100)"
    )
...
class ScoreThresholds(BaseModel):
    ...
    impact_score: Optional[ScoreThreshold] = Field(
        None, description="Filter by impact score range"
    )
    relevance_score: Optional[ScoreThreshold] = Field(
        None, description="Filter by relevance score range"
    )
    novelty_score: Optional[ScoreThreshold] = Field(
        None, description="Filter by novelty score range"
    )
    maturity_score: Optional[ScoreThreshold] = Field(
        None, description="Filter by maturity score range"
    )
    velocity_score: Optional[ScoreThreshold] = Field(
        None, description="Filter by velocity score range"
    )
    risk_score: Optional[ScoreThreshold] = Field(
        None, description="Filter by risk score range"
    )
    opportunity_score: Optional[ScoreThreshold] = Field(
        None, description="Filter by opportunity score range"
    )
...
class SearchFilters(BaseModel):
    ...
    pillar_ids: Optional[List[str]] = Field(
        None, description="Filter by strategic pillar codes (e.g., ['CH', 'HG'])"
    )
    goal_ids: Optional[List[str]] = Field(
        None, description="Filter by goal IDs (e.g., ['CH.1', 'HG.2'])"
    )
    stage_ids: Optional[List[str]] = Field(
        None, description="Filter by maturity stage IDs (e.g., ['6_early_adoption'])"
    )
    ...
    date_range: Optional[DateRange] = Field(
        None, description="Filter by created_at date range"
    )
    score_thresholds: Optional[ScoreThresholds] = Field(
        None, description="Filter by score value ranges"
    )
    status: Optional[str] = Field(
        None, description="Filter by card status (e.g., 'active')"
    )
...
class AdvancedSearchRequest(BaseModel):
    ...
    query: Optional[str] = Field(
        None, max_length=500, description="Search query for text/semantic search"
    )
    filters: Optional[SearchFilters] = Field(
        None, description="Advanced filter criteria"
    )
    use_vector_search: bool = Field(
        True, description="Use vector similarity search (semantic) vs text search"
    )
    limit: int = Field(
...
class SearchResultItem(BaseModel):
    ...
    match_highlights: Optional[List[str]] = Field(
        None, description="Highlighted text snippets showing query matches"
    )
...
class AdvancedSearchResponse(BaseModel):
    ...
    results: List[SearchResultItem] = Field(
        default_factory=list, description="List of matching cards with relevance scores"
    )
    total_count: int = Field(
        0, description="Total number of matching cards (before limit)"
    )
    ...
    search_type: str = Field(
        "vector", description="Type of search performed: 'vector' or 'text'"
    )
...
class SavedSearchUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=100, description="New name for the saved search"
    )
    query_config: Optional[Dict[str, Any]] = Field(
        None, description="Updated search configuration"
    )
...
class SavedSearch(BaseModel):
    ...
    user_id: str = Field(..., description="UUID of the user who owns this saved search")
    name: str = Field(..., description="User-defined name for the saved search")
    query_config: Dict[str, Any] = Field(
        ..., description="Complete search configuration including query and filters"
    )
    created_at: datetime = Field(..., description="When the saved search was created")
    last_used_at: datetime = Field(
        ..., description="When the saved search was last executed"
    )
    updated_at: Optional[datetime] = Field(
        None, description="When the saved search was last modified"
    )
...
class SavedSearchList(BaseModel):
    ...
    saved_searches: List[SavedSearch] = Field(
        default_factory=list, description="List of user's saved searches"
    )
    total_count: int = Field(default=0, description="Total number of saved searches")
...
class SearchHistoryEntry(BaseModel):
    ...
    id: str = Field(..., description="UUID of the history entry")
    user_id: str = Field(..., description="UUID of the user who executed the search")
    query_config: Dict[str, Any] = Field(
        ..., description="Search configuration that was executed"
    )
    executed_at: datetime = Field(..., description="When the search was executed")
    result_count: int = Field(
        default=0, description="Number of results returned by the search"
    )
...
class SearchHistoryList(BaseModel):
    ...
    history: List[SearchHistoryEntry] = Field(
        default_factory=list, description="List of recent search history entries"
    )
    total_count: int = Field(
        default=0, description="Total number of history entries (max 50 per user)"
    )
