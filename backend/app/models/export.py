...
class ChartOptions(BaseModel):
    ...
    include_score_radar: bool = Field(
        True, description="Include radar chart showing all score dimensions"
    )
    include_score_bars: bool = Field(
        True, description="Include bar chart showing individual scores"
    )
    include_pillar_distribution: bool = Field(
        False, description="Include pillar distribution chart (workstream reports only)"
    )
    chart_dpi: int = Field(
        300, ge=72, le=600, description="DPI resolution for chart images (72-600)"
    )
...
class ExportRequest(BaseModel):
    ...
    format: ExportFormat = Field(..., description="Export format: pdf, pptx, or csv")
    include_charts: bool = Field(
        True, description="Include visualizations in PDF/PPTX exports"
    )
    include_description: bool = Field(
        True, description="Include full description text in export"
    )
    include_metadata: bool = Field(
        True, description="Include metadata like created_at, updated_at"
    )
    chart_options: Optional[ChartOptions] = Field(
        None, description="Advanced chart configuration options"
    )
...
class ExportResponse(BaseModel):
    ...
    format: ExportFormat = Field(..., description="Format of the generated export")
    filename: str = Field(..., description="Suggested filename for the export")
    content_type: str = Field(..., description="MIME type of the export file")
    file_size_bytes: Optional[int] = Field(
        None, ge=0, description="Size of the generated file in bytes"
    )
    ...
    card_id: Optional[str] = Field(
        None, description="ID of the exported card (if single card export)"
    )
    card_name: Optional[str] = Field(None, description="Name of the exported card")
    charts_included: bool = Field(
        False, description="Whether charts were included in the export"
    )
...
class WorkstreamExportRequest(BaseModel):
    ...
    include_summary: bool = Field(
        True, description="Include summary page with aggregate statistics"
    )
    include_charts: bool = Field(
        True,
    ...
    include_card_details: bool = Field(
        True, description="Include detailed sections for each card"
    )
    max_cards: int = Field(
        50, ge=1, le=100, description="Maximum number of cards to include in report"
    )
    chart_options: Optional[ChartOptions] = Field(
        None, description="Advanced chart configuration options"
    )
...
class WorkstreamExportResponse(BaseModel):
    ...
    workstream_id: str = Field(..., description="ID of the exported workstream")
    workstream_name: Optional[str] = Field(
        None, description="Name of the exported workstream"
    )
    cards_included: int = Field(
        0, ge=0, description="Number of cards included in the report"
    )
    charts_included: bool = Field(
        False, description="Whether charts were included in the export"
    )
...
class CardExportData(BaseModel):
    ...
    stage_name: Optional[str] = Field(
        None, description="Maturity stage name (deprecated)"
    )
    horizon: Optional[str] = Field(None, description="Time horizon (deprecated)")
    ...
    # Deep Research
    deep_research_report: Optional[str] = Field(
        None, description="Latest deep research report markdown"
    )
